...
namespace SimpleCode.MailSender.Business
{
    public class DisparoBusiness
    {
        ...
        private void Inserir(int codigoAmbiente, int codigoMensagem, int codigoDestinatario, TipoDestinatario tipo, IList<int> copias, StringCollection anexos, Dictionary<string, string> variaveis1, params string[] variaveis2)
        {
            ...
            {
                throw new ArgumentException("A quantidade de variáveis esperada para esta mensagem é maior que a quantidade de valores informados.");
            }
            ...
            for (int i = 0; i < mensagem.Variaveis.Count; i++)
            {
                ...
                if (variaveis2.Length > 0)
                {
                    if (i >= variaveis2.Length)
                        new AlertaInfo(
                            string.Concat("Valor da variável ", mensagem.Variaveis[i].Nome, " nao fornecido."), TipoAlerta.Alerta).Salvar();
                    else
                        valor = variaveis2[i];
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        new AlertaInfo(
                            string.Concat("Valor da variável ", mensagem.Variaveis[i].Nome, " nao fornecido. "), TipoAlerta.Erro, new Exception(string.Concat("Conteudo do dicionario: ", conteudo))).Salvar();
                    }
